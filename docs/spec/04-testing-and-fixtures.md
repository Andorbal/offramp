...
| `hollow` (generated by `GeneratedFixtures.Hollow`, not checked in) | `Big` (`netstandard2.0`, 500 files: 10 folders of 49 chained public types and an internal helper), an empty `Big.Core`, and `App` using Big | the overnight `move plan --all` + `move apply` run with a single verification |
