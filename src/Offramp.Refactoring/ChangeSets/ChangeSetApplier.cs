...
public sealed class ChangeSetApplier(string repositoryRoot, IGitService git)
{
    ...
    /// <summary>Applies the change set and returns the journal's repository-relative path.</summary>
    public async Task<string> ApplyAsync(ChangeSet changeSet, string command, DateTimeOffset now, CancellationToken cancellationToken)
    {
        ...
        steps.AddRange(changeSet.Creates.OrderBy(c => c.Path, StringComparer.Ordinal)
            .Select(c => new JournalStep { Kind = JournalStepKind.Create, Path = c.Path, Sha256 = ContentHash.Sha256(c.Content) }));
        steps.AddRange(changeSet.Edits.OrderBy(e => e.Path, StringComparer.Ordinal)
            .Select(e => new JournalStep { Kind = JournalStepKind.Edit, Path = e.Path, Before = Convert.ToBase64String(e.Before), Sha256 = ContentHash.Sha256(e.After) }));
        steps.AddRange(changeSet.Renames.OrderBy(r => r.From, StringComparer.Ordinal)
            .Select(r => new JournalStep { Kind = JournalStepKind.Rename, Path = r.To, From = r.From, Sha256 = r.Sha256 }));
        ...
        for (var i = 0; i < steps.Count; i++)
        {
            ...
            var created = await PerformAsync(changeSet, steps[i], cancellationToken);
    ...
    private async Task<IReadOnlyList<string>> PerformAsync(ChangeSet changeSet, JournalStep step, CancellationToken cancellationToken)
    {
        switch (step.Kind)
        {
            case JournalStepKind.Create:
            {
                ...
                var file = changeSet.Creates.Single(c => c.Path == step.Path);
                Directory.CreateDirectory(Path.GetDirectoryName(Absolute(step.Path))!);
                await File.WriteAllBytesAsync(Absolute(step.Path), file.Content, cancellationToken);
                return created;
            }

            case JournalStepKind.Edit:
                await File.WriteAllBytesAsync(Absolute(step.Path), changeSet.Edits.Single(e => e.Path == step.Path).After, cancellationToken);
                return [];

            default:
            {
                var created = MissingDirectories(step.Path);
                if (ContentHash.Sha256File(Absolute(step.From!)) != step.Sha256)
                {
                    throw new PurityViolationException($"{step.From} changed after it was planned.");
                }

                await git.MoveAsync(repositoryRoot, Absolute(step.From!), Absolute(step.Path), cancellationToken);
                if (ContentHash.Sha256File(Absolute(step.Path)) != step.Sha256)
                {
